...
language_error_t name_table_ctor(language_t *ctx, size_t capacity) {
    ...
    //--------------------------------------------------------------------------------//
    ctx->name_table.identifiers = (identifier_t *)calloc(capacity, sizeof(ctx->name_table.identifiers[0]));
...
language_error_t set_memory_addr(language_t *ctx, language_node_t *node, size_t addr) {
    _C_ASSERT(ctx  != NULL, return LANGUAGE_CTX_NULL );
...
language_error_t used_names_ctor(language_t *ctx, size_t capacity) {
    _C_ASSERT(ctx != NULL, return LANGUAGE_CTX_NULL);
    //--------------------------------------------------------------------------------//
    ctx->name_table.used_names = (name_t *)calloc(capacity, sizeof(ctx->name_table.used_names[0]));
...
language_error_t used_names_add(language_t *ctx, const char *name, size_t length, size_t *index) {
    _C_ASSERT(ctx  != NULL, return LANGUAGE_CTX_NULL );
