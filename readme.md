...
Во Front-end'е происходит преобразование исходного кода на языке KVM в абстрактное синтаксическое дерево (AST).
...
Middle-end производит оптимизации над AST. В этом компиляторе представлены следующий оптимизации:
- Свёртка констант (Вычисление значения выражения, где это возможно)
- Удалений нейтральных операций (Позволяет, например, не производить умножение на 1)
...
Для примера далее представлены преобразования выполненные компилятором для кода, написанного на языке **KVM**, выполняющего вычисление факториала. В первой функции созданы две неиспользуемые переменные, про них будет сказано позже, в момент обсуждения оптимизации.
...
Абстрактное синтаксическое дерево, полученное после работы Front-end'а представлено на рисунке ниже. Как видно, вычисления, связанные с инициализацией переменных unused и unused_ остались в дереве(на рисунке они выделены красным и зелёным прямоугольниками).
...
Далее в Middle-end'е производится свёртка констант, а также упрощение действий с нейтральными элементами. В функции факториала присутствуют две переменные, инициализация которых может быть оптимизированна.

Одна из переменных инициализируется выражением состоящим только из констант (выделено красным прямоугольником), оно может быть вычислено на этапе компиляции. Другая переменная задаётся выражением, которое не может быть вычислено на этапе компиляции, так как содержит вычисления, связанные с идентификатором (выделено зелёным прямоугольником). Однако в ходе вычислений происходит умножение на 0, поэтому эти вычисления также могут быть оптимизированны.

Графическое представление результата работы Middle-end'а представлено на рисунке ниже. Соответствующие выражения выделены прямоугольниками тех же цветов.
...
В Back-end'е первым делом создаётся промежуточное представление [**IR**](#представление-в-виде-ir), ниже представлен его графический дамп.
<details>
...
Далее в Back-end'е происходят оптимизации над [**IR**](#представление-в-виде-ir), ниже представлен графический дамп оптимизированного промежуточного представления, зелёным цветом выделены оптимизированные узлы.
<details>
